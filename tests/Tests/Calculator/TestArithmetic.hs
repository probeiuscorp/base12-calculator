...
tests1 = testGroup "arithmetic"
  ...
  where
    ...
    simplifyCase ratio@CalcValue{..} = snapshot @C.System "simplify" (show valNumerator <> "÷" <> show valDenominator) 120 $
      let
        bRatio = fromEdges Nothing [(6, Just ratio), (7, Nothing)]
        bSimplified = simplifyHandshake bRatio
      in
    ...
    prettyPrintOp = \case
      ...
      ArithMul -> "×"
      ArithDiv -> "÷"
    arithmeticCase op a b = snapshot @C.System "arithmetic" (unwords [show a, prettyPrintOp op, show b]) 80 $
      let
        bmAction = fromEdges Nothing [(6, Just op), (7, Nothing)]
        (bmError, bmResult) = arithmetic (pure $ Just a) (pure $ Just b) bmAction
      in
        [ raw "action" $ bmAction ## maybe "-" show
        , col "err" bmError
        , col "result" bmResult
        ]
